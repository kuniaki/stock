...
def api_revenue():
    ...
    annual_revenue = investpy.get_stock_financial_summary(
        stock=code, country=country, summary_type='income_statement', period='annual')
    revenue_a = annual_revenue.reset_index()

    quarterly_revenue = investpy.get_stock_financial_summary(
        stock=code, country=country, summary_type='income_statement', period='quarterly')
...
def kabuka(code, S_year, S_day):
    ...
    df_base.index = pd.DatetimeIndex(
        df_base.timestamp, name='timestamp').tz_localize('UTC').tz_convert('Asia/Tokyo')
...
def grabFromUrl(code):
    ...
    # issue an HTTP GET requests to given URL
    kabutan_overview_page = requests.get(kabutan_overview_url)
    ...
    kabutan_overview_soup = BeautifulSoup(
        kabutan_overview_page.content, "html.parser")
    kabutan_news_soup = BeautifulSoup(kabutan_news_page.content, "html.parser")
    kabutan_disclosure_soup = BeautifulSoup(
        kabutan_disclosure_page.content, "html.parser")
    kabutan_capital_soup = BeautifulSoup(
        kabutan_capital_page.content, "html.parser")
...
def grabOverviewSoup(soup):
    ...
    company_per = soup.select('div#stockinfo_i3 > table > tbody > tr > td')[
        0].get_text()
    company_marketvalue = soup.select(
        'div#stockinfo_i3 > table > tbody > tr > td.v_zika2')[0].get_text()
    ...
    result = dict(
        {company[0]: company[1], "PER": company_per, "時価総額": company_marketvalue})
...
def grabNewsSoup(soup, index):
    ...
    time_list = list(str(i).replace(
        'news_time', 'news-time').replace('\xa0', ' ') for i in time)
    ...
    for date in news_table:
        ...
        for link in date.children:
            if "href" in str(link):
                if index == 2:
                    news['link'].append(str(link).replace(
                        '/stock/news?', 'https://kabutan.jp/stock/news?').replace("&amp;", "&"))
                elif index == 3:
                    news['link'].append(str(link).replace('<img alt="pdf" src="/images/cmn/pdf16.gif"/>',
                                        '').replace(' target="pdf"', '').replace(' class="td_kaiji"', ''))
...
def grabCapitalSoup(soup):
    ...
    for c in capital_table:
        if (count == len(capital_table) - 1):
            result.append(str(c).replace(str(c.select('td')[0]), '').replace(str(c.select('td')[-1]), '').replace(
                str(c.select('td')[1]), '').replace('\n</tr></tr>', '').replace('<tr><tr>\n', '').replace("\n\n", "\n"))
        else:
            print(c)
            result.append(str(c).replace("/holder/lists/?", "https://kabutan.jp/holder/lists/?").replace(
                str(c.select('td')[-1]), '').replace(str(c.select('td')[0]), ''))
        count += 1
    return result
...
def grabNewsUlletSoup(soup, item):
    ...
    for item in ullet_news:
        temp = str(item).replace('<img alt="link.gif" height="13" src="https://www.ullet.com/img/icon/link.gif?218" width="13"/>',
                                 '').replace('<h3>', '').replace('･</h3>', '').split('</a> ')
        result['link'].append('<td>' + temp[0] + '</a></td>')
        result['date'].append(
            '<td class="ullet-news-time">' + temp[1] + '</td>')
    return result
